...
REST_ROUTER.prototype.handleRoutes = function (router, connection, md5) {
	...
	function registrarDetalleNormal(res, programacionDiariaCab_id, detalleNormal) {
		...
		registrarDetalleSobrante(programacionDiariaCab_id);
	}

	function registrarDetalleSobrante(programacionDiariaCab_id) {
		connection.query(`
			...
			WHERE   PCAB.id = ` + programacionDiariaCab_id, function (res, err, result) {
				if (err) {
					res.json({ "error": err });
				} else {
					// registrarDetallePedido(programacionDiariaCab_id);
				}
		});
	}
/* 
	function registrarDetallePedido(res, programacionDiariaCab_id) {
		connection.query(`
			...
			SELECT	PCAB.id,
					PED.torta_id,
					PED.tamano_id
			FROM	pedido PED INNER JOIN programacionDiariaCab PCAB ON PED.fecha = DATE_ADD(PCAB.fecha, INTERVAL -1 DAY) AND
									                                          PED.sucursal_id = PCAB.sucursal_id
	...
	function registrarDetalleEspecial(res, programacionDiariaCab_id) {
		connection.query(`
			...
			SELECT	PCAB.id,
					ESP.id
			FROM	pedidoEspecial ESP INNER JOIN programacionDiariaCab PCAB ON ESP.fecha = DATE_ADD(PCAB.fecha, INTERVAL -1 DAY) AND
